...
#+begin_src sh :var path=empath[0] :tangle emacs_starter.sh  :shebang #!/bin/bash :dir /sudo:: 
  emacs -q -l $path/init.el \
      --eval "(run-hooks 'after-init-hook)" &
#+end_src

#+BEGIN_SRC sh :var path=empath[0]   :dir /sudo::  
ls -la $path/emacs_starter.sh 
cp $path/emacs_starter.sh  /bin/
chmod 755 /bin/emacs_starter.sh
ls -la /bin/emacs_starter.sh
#+END_SRC

#+RESULTS:
| -rwxr-xr-x | 1 | rks  | rks  | 151 | Jan | 6 | 02:56 | /home/rks/literate-emacs/emacs_starter.sh |
| -rwxr-xr-x | 1 | root | root | 151 | Jan | 6 | 03:05 | /bin/emacs_starter.sh                     |
